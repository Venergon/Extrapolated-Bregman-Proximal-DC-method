function [x] = argmin_soft_threshold(df, dD, w, xi, L, t, x_prev, lambda, thresholding_iterations, transform, itransform)
...
for iteration = 1:thresholding_iterations
    inner_vector = transform(x - step_size*dh(x));

    x = sign(inner_vector).*max((abs(inner_vector) - lambda*step_size), 0);
    x = itransform(x);
end

obj_fn = @(x) (lambda*sum(abs(transform(x)), 'all') + trace((df_w - xi)'*(x-w)) + L/2*(norm(x-w, 'fro')^2) + (1/t) * (1/2) * (norm(x-x_prev, 'fro')^2));
